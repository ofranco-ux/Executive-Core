...
def procesar_archivo_excel(file_source, target_sl=80.0, target_time=20.0, merma=0.20, dias_futuros=30):
    ...
    if pd.isna(fecha_maxima): fecha_maxima = pd.to_datetime(datetime.now()).normalize()
    ...
    if pd.isna(max_date_hist): df_reciente = df_filtrado.copy()
    else: df_reciente = df_filtrado[df_filtrado[col_fecha] >= (max_date_hist - timedelta(days=28))]
