...
def resolver_turnos_optimos(intervalos, campanas_activas, llamadas_vec=None, aht_vec=None, req_vec=None, target_sl=80.0, target_time=20.0, merma=0.20, duracion_jornada=8.0, es_nocturno=False, es_outbound=False, es_chat=False, concurrencia=3.0):
    ...
    if len(valid_shifts) > 0:
        for _ in range(5000): 
            ...
            best_idx = -1
            best_reduction = 0.001
            
            # Dinamismo: Si ya llegamos al SL, no permitimos un overstaff agresivo.
            ...
            current_under = np.maximum(0, req_hc_base - cob_hc)
            current_over = np.maximum(0, cob_hc - req_hc_base)
            current_cost = np.sum((current_under ** 2) * w_under) + np.sum((current_over ** 2) * w_over)
            ...
            for idx, sh in enumerate(valid_shifts):
                ...
                new_cost = np.sum((new_under ** 2) * w_under) + np.sum((new_over ** 2) * w_over)
        ...
        # El Buddy system interviene, pero solo si es absolutamente necesario (no duplica turnos gigantescos)
        for idx in range(len(shift_counts)):
            if shift_counts[idx] == 1:
                # Solo asigna el buddy si ese horario particular tiene alto volumen real en la base
                arr = valid_shifts[idx]['arr']
                max_req_in_shift = np.max(req_hc_base * arr)
                if max_req_in_shift > 1:
                    shift_counts[idx] = 2
